...
class ValidateTeaForm(FormValidationAction):
    ...
    def validate_tea_type(
        ...
        if slot_value.lower() not in ALLOWED_TEA_TYPES:
            dispatcher.utter_message(text=f"Xin lỗi quý khách, cửa hàng mình chưa bán {slot_value} ạ")
            return {"tea_type": None}
        dispatcher.utter_message(text=f"Xác nhận bạn muốn mua {slot_value}.")
    ...
    def validate_tea_size(
        ...
        if slot_value not in ALLOWED_TEA_SIZES:
            dispatcher.utter_message(text=f"Chúng mình chỉ có size Vừa và size Lớn thôi ạ.")
    ...
    def validate_tea_quan(
        ...
        if int(slot_value) < 1 or int(slot_value) > 99:
            dispatcher.utter_message(text=f"Rất xin lỗi quý khách chúng mình chỉ có thế phục vụ tối thiểu là 1 và tối đa là 99 sản phẩm thôi ạ")
...
class action_return_tea_price(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        tea_type = 'loại sản phẩm quý khách vừa yêu cầu'
        ...
        if price:
            ...
            dispatcher.utter_message(text=f"Xin lỗi quý khách, cửa hàng mình chưa phục vụ {tea_type} ạ")
            return []
...
class action_inquired_tea_type(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        tea_type = tracker.get_slot("inquired_tea_type").lower() if tracker.get_slot("inquired_tea_type") else 'loại sản phẩm quý khách vừa yêu cầu'
        ...
        if tea_type in ALLOWED_TEA_TYPES:
            dispatcher.utter_message(text=f"dạ cửa hàng mình có {tea_type} ạ")
            return []
        else:
            dispatcher.utter_message(text=f"Xin lỗi quý khách, cửa hàng mình chưa phục vụ {tea_type} ạ")
            return []
